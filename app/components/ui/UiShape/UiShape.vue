<template>
	<span class="ui-shape" :class="shapeIconClasses" :style="shapeIconStyles" aria-hidden="true">
		<svg viewBox="0 0 50 52" fill="none" xmlns="http://www.w3.org/2000/svg" class="ui-shape__bg">
			<path
				...
				class="ui-shape__bg-path" />
		</svg>

		<span class="ui-shape__content">
			<slot>
				<UiIcon v-if="icon" :name="icon" class="ui-shape__content-icon" />
			</slot>
		</span>
	</span>
</template>
...
<script setup lang="ts">
	...
	const shapeIconClasses = computed(() => ({
		[`ui-shape--${props.position}`]: props.position,
	}))

	const shapeIconStyles = computed(() => ({
		'--ui-shape-width': toCssValue(props.width),
		'--ui-shape-height': toCssValue(props.height),
		'--ui-shape-color': props.color || undefined,
		'--ui-shape-inner-width': toCssValue(props.iconWidth),
		'--ui-shape-inner-height': toCssValue(props.iconHeight),
		'--ui-shape-inner-color': props.iconColor || undefined,
	}))
</script>
